...
final class ProductCategoriesViewModel: FormViewModel {

    // MARK: - Callbacks
    var onTapProduct: ((CommodityCategoryResponse) -> Void)?
    var onToggleFavorite: ((Bool, CommodityCategoryResponse) -> Void)?
        ...
        let filtered = await withCheckedContinuation { continuation in
            DispatchQueue.global(qos: .userInitiated).async {
                let results = self.state.products.filter { product in
                    ...
                    (product.description?.lowercased().contains(lowercasedSearch) ?? false)
                }
    ...
    // MARK: - Builders
    private func makeProductGridItems() -> [ImageTitleGridItemModel] {
    ...
    private func loadProducts(reset: Bool) async -> Bool {
        do {
            let response = try await commonUtilitiesService.getCommodityCategory(
                page: state.currentPage, count: state.itemsPerPage, module: "<regulation | trade-documents | standards>", accessToken: state.accessToken)
    ...
    private lazy var productGridRow = ImageTitleGridFormRow(
        tag: CellTag.productList.rawValue,
        items: makeProductGridItems(),
        numberOfColumns: 2
    )
