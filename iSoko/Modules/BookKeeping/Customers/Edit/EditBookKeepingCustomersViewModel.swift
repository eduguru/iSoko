//
//  EditBookKeepingCustomersViewModel.swift
//  
//
...
final class EditBookKeepingCustomersViewModel: FormViewModel {
    ...
    init(customer: CustomerResponse) {
        self.state = State(customer: customer)
        super.init()
        sections = makeSections()
        prefill()
    }
    
    // MARK: - Prefill (IMPORTANT PART YOU WERE MISSING)
    private func prefill() {
        customerNameInputRow.model.text = state.customerName
    ...
    // MARK: - Submit (ONLY DIFFERENCE: update API)
    ...
    private func performNetworkRequest() async -> Bool {
        ...
        do {
            _ = try await bookKeepingService.updateCustomer(
                itemId: state.customer.id,
                parameters: payload,
    ...
    private struct State {
        ...
        init(customer: CustomerResponse) {
            ...
            self.emailAddress = customer.email ?? ""
            self.town = customer.town ?? ""
            self.streetAddress = customer.streetAddress ?? ""
        }
    }
    
    // MARK: - Tags 
    enum Tags {
        enum Section: Int { case header = 0 }
